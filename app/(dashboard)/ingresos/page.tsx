    ...
    <div className="flex flex-col gap-6 p-6 pb-10">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="col-span-2 rounded-xl border border-emerald-500/20 bg-gradient-to-br from-emerald-500/10 to-transparent p-5">
          ...
          <p className="mt-1 text-xs text-white/35">2024 + 2025 combinado</p>
        </div>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#111118] p-5">
          ...
          <div className="mt-3 space-y-1">
            ...
            <div className="flex justify-between text-xs">
              <span className="text-white/40">Vía Fénix Academy</span>
              <span className="text-white/30">LLC</span>
            </div>
          </div>
        </div>
      </div>
              ...
              <thead>
                <tr className="border-b border-white/[0.06]">
                  {["Fecha","Fuente","Descripción","Tipo","Estado","Monto"].map((h, i) => (
                    <th key={i} className={`py-3 text-left text-[11px] font-medium uppercase tracking-wider text-white/40 ${i === 0 ? 'pl-4' : 'px-3'} ${i === 5 ? 'pr-4 text-right' : ''}`}>{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {items.length === 0 && (
                  <tr><td colSpan={6} className="py-8 text-center text-sm text-white/30">Sin registros</td></tr>
                )}
                {items.map((ing) => (
