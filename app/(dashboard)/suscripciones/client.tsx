...
import { Filter, CreditCard, CheckCircle2, XCircle, Plus, Pencil, Trash2 } from "lucide-react";
import {
...
// ─── Constants ───────────────────────────────────────────────
const CATEGORIAS = ["TODAS", "IA", "MARKETING", "COMUNICACION", "VENTAS", "OPERACIONES", "DISEÑO"] as const;
const ESTADOS = ["TODOS", "ACTIVO", "CANCELADO"] as const;
...
function EstadoBadge({ estado }: { estado: string }) {
  return estado === "ACTIVO"
    ? <span className="inline-flex items-center gap-1.5 rounded-full bg-emerald-500/10 px-2.5 py-0.5 text-[11px] font-medium text-emerald-400 ring-1 ring-emerald-500/20"><span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />ACTIVO</span>
    : <span className="inline-flex items-center gap-1.5 rounded-full bg-red-500/10 px-2.5 py-0.5 text-[11px] font-medium text-red-400 ring-1 ring-red-500/20"><span className="h-1.5 w-1.5 rounded-full bg-red-400" />CANCELADO</span>;
}
...
function CategoriaBadge({ cat }: { cat: string }) {
  const map: Record<string, string> = {
    IA: "bg-purple-500/10 text-purple-400", MARKETING: "bg-pink-500/10 text-pink-400",
    COMUNICACION: "bg-sky-500/10 text-sky-400", VENTAS: "bg-emerald-500/10 text-emerald-400",
    OPERACIONES: "bg-amber-500/10 text-amber-400", DISEÑO: "bg-rose-500/10 text-rose-400",
  };
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div className="space-y-1">
          ...
          <select value={form.categoria} onChange={sel("categoria")} className={selectCls}>
            {["IA","MARKETING","COMUNICACION","VENTAS","OPERACIONES","DISEÑO"].map(c => <option key={c}>{c}</option>)}
          </select>
        </div>
        <div className="space-y-1">
        ...
        <div className="space-y-1">
          ...
          <select value={form.estado} onChange={sel("estado")} className={selectCls}>
            <option>ACTIVO</option><option>CANCELADO</option>
          </select>
        </div>
        <div className="space-y-1">
...
export function SuscripcionesClient({ data }: { data: Suscripcion[] }) {
  ...
  const canceladas = data.filter((s) => s.estado === "CANCELADO");
  const totalMensual = activas.reduce((sum, s) => sum + s.costo_mensual, 0);
  const totalFiltrado = filtradas.filter(s => s.estado === "ACTIVO").reduce((sum, s) => sum + s.costo_mensual, 0);

  function handleAdd(formData: SuscripcionData) {
    startTransition(async () => {
      const res = await addSuscripcion(formData);
  ...
  function handleEdit(formData: SuscripcionData) {
    if (!editItem) return;
    startTransition(async () => {
      const res = await updateSuscripcion(editItem.id, formData);
      if (res.error) { toast.error(res.error); return; }
    ...
    <div className="flex flex-col gap-6 p-6 pb-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-white/40">Gestión de herramientas y servicios SaaS</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        <div className="rounded-xl border border-orange-500/20 bg-gradient-to-br from-orange-500/10 to-transparent p-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-white/40">Inversión mensual</p>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#111118] p-4">
          <p className="text-[11px] font-medium uppercase tracking-wider text-white/40">Total apps</p>
          <p className="mt-1.5 text-xl font-bold text-white">{data.length}</p>
        ...
        <div className="rounded-xl border border-white/[0.06] bg-[#111118] p-4">
          ...
          <p className="mt-1.5 text-xl font-bold text-white">{activas.length}</p>
        </div>
        <div className="rounded-xl border border-white/[0.06] bg-[#111118] p-4">
          <div className="flex items-center gap-2">
            <XCircle className="h-4 w-4 text-red-400" />
            <p className="text-[11px] font-medium uppercase tracking-wider text-white/40">Canceladas</p>
          </div>
          <p className="mt-1.5 text-xl font-bold text-white">{canceladas.length}</p>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <div className="flex flex-wrap gap-1.5">
          {CATEGORIAS.map((cat) => (
            <button key={cat} onClick={() => setCatFiltro(cat)} className={cn("rounded-full px-3 py-1 text-xs font-medium transition-all", catFiltro === cat ? "bg-orange-500 text-white" : "bg-white/[0.05] text-white/50 hover:bg-white/[0.08] hover:text-white/80")}>{cat}</button>
          ))}
        </div>
        <div className="mx-2 h-4 w-px bg-white/[0.08]" />
        <div className="flex flex-wrap gap-1.5">
          {ESTADOS.map((est) => (
            <button key={est} onClick={() => setEstFiltro(est)} className={cn("rounded-full px-3 py-1 text-xs font-medium transition-all", estFiltro === est ? "bg-white/15 text-white" : "bg-white/[0.05] text-white/50 hover:bg-white/[0.08] hover:text-white/80")}>{est}</button>
          ))}
        </div>
      ...
      <div className="rounded-xl border border-white/[0.06] bg-[#111118] overflow-hidden">
        <Table>
          ...
          <TableBody>
            {filtradas.map((s) => (
              <TableRow key={s.id} className={cn("border-white/[0.04] transition-colors", s.estado === "CANCELADO" && "opacity-50")}>
                ...
                <TableCell className="pr-4">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => setEditItem(s)} className="rounded p-1 text-white/30 hover:bg-white/[0.06] hover:text-white/70 transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                    <button onClick={() => setDeleteId(s.id)} className="rounded p-1 text-white/30 hover:bg-red-500/10 hover:text-red-400 transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
        ...
        {filtradas.length > 0 && (
          <div className="flex items-center justify-between border-t border-white/[0.06] px-4 py-3">
            <span className="text-xs text-white/40">{activas.length} activas · {canceladas.length} canceladas</span>
      ...
      <Dialog open={!!deleteId} onOpenChange={(o) => !o && setDeleteId(null)}>
        <DialogContent className="border-[#24243a] bg-[#111118] text-white max-w-sm">
          <DialogHeader>
            <DialogTitle className="text-white">¿Eliminar suscripción?</DialogTitle>
          </DialogHeader>
