...
const CATEGORIAS = ["MENTORIA", "VIAJE", "SALIDA", "COMPRA", "HERRAMIENTA", "OTRO"] as const;
type Categoria = typeof CATEGORIAS[number];
const TODAS = ["TODAS", ...CATEGORIAS] as const;
const MONEDAS = ["USD", "MXN"];

const CAT_STYLE: Record<Categoria, string> = {
  MENTORIA: "bg-violet-500/10 text-violet-400 ring-violet-500/20",
  VIAJE: "bg-sky-500/10 text-sky-400 ring-sky-500/20",
  SALIDA: "bg-emerald-500/10 text-emerald-400 ring-emerald-500/20",
  COMPRA: "bg-amber-500/10 text-amber-400 ring-amber-500/20",
  HERRAMIENTA: "bg-orange-500/10 text-orange-400 ring-orange-500/20",
  OTRO: "bg-white/5 text-white/40 ring-white/10",
};

const CAT_ICON: Record<Categoria, string> = {
  MENTORIA: "🎓", VIAJE: "✈️", SALIDA: "🍽️",
  COMPRA: "🛒", HERRAMIENTA: "🔧", OTRO: "💸",
};
...
function getMonthFromFecha(fecha: string) {
  // fecha can be "15/04/2026" or "2026-04-15"
  if (fecha.includes("/")) {
    ...
    <form onSubmit={(e) => { e.preventDefault(); onSubmit(form); }} className="space-y-3">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1 col-span-2">
          ...
          <Input value={form.descripcion} onChange={(e) => set("descripcion", e.target.value)} required placeholder="Ej: Mentoría con Juan, Vuelo CDMX..." className="h-8 bg-[#1a1a24] border-[#24243a] text-white text-sm" />
        </div>
        ...
        <div className="space-y-1">
          ...
          <Input value={form.fecha} onChange={(e) => set("fecha", e.target.value)} required placeholder="15/04/2026" className="h-8 bg-[#1a1a24] border-[#24243a] text-white text-sm" />
        </div>
        <div className="space-y-1">
...
export function GastosPersonalesClient({ data }: { data: GastoPersonal[] }) {
  ...
  const estesMes = data.filter(g => getMonthFromFecha(g.fecha) === currentMonth);
  ...
  const filtered = catFiltro === "TODAS" ? data : data.filter(g => g.categoria === catFiltro);
    ...
    <div className="flex flex-col gap-6 p-6 pb-10">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="mt-0.5 text-sm text-white/40">Mentorias, viajes, salidas y gastos fuera del negocio</p>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        {porCategoria.slice(0, 2).map(({ cat, total }) => (
          <div key={cat} className="rounded-xl border border-white/[0.06] bg-[#111118] p-4">
            <div className="mb-2 flex items-center gap-2">
            ...
            <p className="mt-0.5 text-[11px] text-white/35">{data.filter(g => g.categoria === cat).length} gastos</p>
          </div>
        ))}
      </div>

      {/* Categoria breakdown chips */}
      <div className="flex flex-wrap gap-2">
        {porCategoria.filter(x => x.total > 0).map(({ cat, total }) => (
          <div key={cat} className={cn("flex items-center gap-2 rounded-full px-3 py-1.5 text-xs font-medium ring-1", CAT_STYLE[cat])}>
        ...
        <div className="flex flex-wrap gap-1.5">
          {TODAS.map((cat) => (
            <button key={cat} onClick={() => setCatFiltro(cat)}
              className={cn("rounded-full px-3 py-1 text-xs font-medium transition-all", catFiltro === cat ? "bg-orange-500 text-white" : "bg-white/[0.05] text-white/50 hover:bg-white/[0.08] hover:text-white/80")}>
              {cat !== "TODAS" && <span className="mr-1">{CAT_ICON[cat as Categoria]}</span>}{cat}
            </button>
          ))}
        </div>
        <span className="ml-auto text-xs text-white/30">{filtered.length} resultado{filtered.length !== 1 ? "s" : ""}</span>
      </div>
      ...
      <div className="rounded-xl border border-white/[0.06] bg-[#111118] overflow-hidden">
          ...
          <TableHeader>
            <TableRow className="border-white/[0.06] hover:bg-transparent">
              {["Fecha","Categoría","Descripción","Monto","Moneda","Notas",""].map((h, i) => (
                <TableHead key={i} className={cn("text-white/40 text-[11px] font-medium uppercase tracking-wider", i === 0 && "pl-4", i === 6 && "pr-4 w-16")}>{h}</TableHead>
              ))}
            </TableRow>
          </TableHeader>
          <TableBody>
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={7} className="py-12 text-center">
                  ...
                  <p className="text-sm text-white/40">Sin gastos personales registrados</p>
                  <p className="text-xs text-white/25 mt-1">Agrega mentorias, viajes, salidas…</p>
                </TableCell>
              </TableRow>
            )}
            {filtered.map((g) => (
              <TableRow key={g.id} className="border-white/[0.04]">
                <TableCell className="pl-4"><span className="text-xs text-white/60 font-mono">{g.fecha}</span></TableCell>
                <TableCell>
                  <span className={cn("inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-[11px] font-medium ring-1", CAT_STYLE[g.categoria as Categoria] ?? CAT_STYLE.OTRO)}>
                    <span>{CAT_ICON[g.categoria as Categoria] ?? "💸"}</span>{g.categoria}
                  </span>
                </TableCell>
                <TableCell><span className="text-sm text-white">{g.descripcion}</span></TableCell>
                <TableCell><span className="text-sm font-bold text-white">{fmt(g.monto, g.moneda)}</span></TableCell>
                <TableCell><span className="rounded bg-white/[0.06] px-1.5 py-0.5 text-[10px] font-mono text-white/50">{g.moneda}</span></TableCell>
                <TableCell><span className="text-xs text-white/40">{g.notas ?? "—"}</span></TableCell>
                <TableCell className="pr-4">
                  <div className="flex items-center justify-end gap-1">
                    <button onClick={() => setEditItem(g)} className="rounded p-1 text-white/30 hover:bg-white/[0.06] hover:text-white/70 transition-colors"><Pencil className="h-3.5 w-3.5" /></button>
                    <button onClick={() => setDeleteId(g.id)} className="rounded p-1 text-white/30 hover:bg-red-500/10 hover:text-red-400 transition-colors"><Trash2 className="h-3.5 w-3.5" /></button>
                  </div>
                </TableCell>
              </TableRow>
            ))}
        ...
        {filtered.length > 0 && (
          <div className="flex items-center justify-between border-t border-white/[0.06] px-4 py-3">
            <span className="text-xs text-white/40">{filtered.length} gasto{filtered.length !== 1 ? "s" : ""} mostrados</span>
            <span className="text-sm font-bold text-orange-400">{fmt(filtered.reduce((s, g) => s + g.monto, 0))}</span>
          </div>
        )}
      </div>
      ...
      <Dialog open={!!editItem} onOpenChange={(o) => !o && setEditItem(null)}>
        <DialogContent className="border-[#24243a] bg-[#111118] text-white max-w-md">
          ...
          {editItem && (
            <GastoForm
              initial={{ fecha: editItem.fecha, categoria: editItem.categoria, descripcion: editItem.descripcion, monto: editItem.monto, moneda: editItem.moneda, notas: editItem.notas ?? "" }}
              onSubmit={handleEdit}
              isPending={isPending}
            />
          )}
